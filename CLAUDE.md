...
- **Sistema de disseny**: `frontend/src/assets/base.css` defineix els tokens globals — paletes (`--verd-*`, `--nit-*`, `--pedra-*`), semàntics (`--color-fons`, `--color-superficie`, `--color-vora`, `--color-text-*`), radis (`--radi-xs..lg`, `--radi-pastilla`), ombres en capes (`--ombra-1..4`), moviment (`--mou-rapid/mig/pop`) i tipografies (`--font-ui` = Inter Variable, `--font-display` = Outfit Variable, autoallotjades via `@fontsource-variable/*` importades a `main.css`). Els components nous han de consumir aquests tokens en lloc de valors literals, i tota animació ha de portar el seu bloc `prefers-reduced-motion`.
